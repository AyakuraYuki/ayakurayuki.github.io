...
import { fileAtSlot, fileLocation as referenceLocation } from "../../vendor/rhine/src/data";
...
import { archiveFraming, swipeDirection } from "../../vendor/rhine/src/viewport-layout";
...
export class ArchiveScene {
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    if (this.disposed) { disposeObject(gltf.scene); throw new Error("Archive scene disposed during loading"); }
      ...
      this.onHover?.(
        hit
          ? hit.instanceId !== undefined
            ? fileAtCell(this.cells[hit.instanceId], this.coordinateOrigin)
            : this.looping ? this.selectedIndex : fileAtSlot(this.selectedSlot)
          : null,
      );
    });
  ...
  dispose() {
    ...
    this.assemblyTemplate?.then(template => disposeObject(template)).catch(() => {});
    this.renderer.dispose(); this.renderer.forceContextLoss(); this.renderer.domElement.remove();
    this.scene.clear(); this.instances = []; this.outgoing = []; this.loaded = false;
  }
}
