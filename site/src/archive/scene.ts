...
import { ArchiveVisibility } from "../../vendor/rhine/src/archive-visibility";
import { disposeThreeTree } from "../../vendor/rhine/src/three-resources";
import { ThemeWave } from "../../vendor/rhine/src/theme-motion";
import { themeMaterial, themeEnvironment } from "../../vendor/rhine/src/theme-material";
import { RhythmMotion, rhythmDisplacement, quietBands, type MusicBands, type RhythmStyle } from "../../vendor/rhine/src/archive-play-motion";
import { GLTFLoader } from "three/addons/loaders/GLTFLoader.js";
import { createArchiveLighting, type LightingLook } from "../../vendor/rhine/src/archive-lighting";
...
import { normalizeQuality, type RenderQuality } from "../../vendor/rhine/src/render-quality";
import { applyTextureQuality, resizeQuality } from "../../vendor/rhine/src/quality-renderer";
import { CardAppearance } from "../../vendor/rhine/src/appearance";
import { configureInternalOptics } from "../../vendor/rhine/src/internal-optics";
import { DecryptionController } from "../../vendor/rhine/src/decryption";
import { records, fileLocation } from "./data";
import { fileLocation as referenceLocation } from "../../vendor/rhine/src/data";
import {
  cellKey,
  sameCell,
  selectionCell,
...
import { labelMarkSvg } from "../../vendor/rhine/src/brand";
import { archiveFraming } from "../../vendor/rhine/src/viewport-layout";
import { ArchiveDrag, ArchivePlaneMomentum, type DragAxis, type DragProjection, type DragPosition } from "../../vendor/rhine/src/archive-drag";
import { assetUrl as publicAsset } from "../../vendor/rhine/src/asset-url";
import {
  archiveWave,
  extraction,
...
export class ArchiveScene {
  ...
  dispose() {
    ...
    this.disposed = true;
    this.onSelect = this.onHover = this.onNavigate = undefined;
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    if (this.disposed) { disposeThreeTree(gltf.scene); throw new Error("Archive disposed while loading"); }
  ...
  setMode(mode: "hidden" | "archive" | "detail") {
    ...
    if (!this.looping) {
      const canonical = referenceLocation(0);
  ...
  select(index: number, navigation?: ArchiveNavigation) {
    ...
    const canonical = this.looping ? fileLocation(index) : referenceLocation(0);
    const next = canonical.slot;
    const cell = this.looping
      ? selectionCell(index, this.selectedCell, navigation, this.coordinateOrigin)
    ...
    this.targetRotation = 0;
    this.drawLabel(this.looping ? index : 0);
  }
  ...
  private drawLabel(index: number) {
    ...
    c.fillText("NO." + (this.looping ? records[index].code.slice(2) : "001"), 22, 360);
  ...
  private navigatePlane(coordinate: DragPosition) {
    ...
    try {
      ...
      for (let i = 1; i <= steps; i++) {
        ...
        if (!sameCell(cell, this.selectedCell)) this.onSelect?.(fileAtCell(cell, this.coordinateOrigin), cell);
      }
    } finally {
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      if (!cancelled && browse && this.canBrowse()) {
        ...
        if (this.archiveDrag.active) {
          ...
          const cell = this.pickCell(e.clientX, e.clientY);
          if (cell) this.onSelect?.(fileAtCell(cell, this.coordinateOrigin), cell);
        }
      }
      reset();
    }, { signal: this.inputEvents.signal });
  ...
  getStats() {
    ...
    return {
      ...
      selectedLane: this.looping ? records[this.selectedIndex].lane : Math.floor(this.selectedSlot / 32),
      selectedPostId: this.looping ? records[this.selectedIndex].id : null,
      selectedMappedIndex: this.looping ? fileAtCell(this.selectedCell, this.coordinateOrigin) : null,
