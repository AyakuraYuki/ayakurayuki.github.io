...
本文件是维护说明，不会打包进公开站点。2026-09-10 用户已告知手动执行首次发布；本次状态整理未核验部署结果，不将“已触发”记作“已上线”，也不重复触发。后续直接在原博客目录的 `codex/rhine-blog` 分支开发，`rhine-blog` 为用户设立的备份／主线变更保存分支。
...
- 旧 Hugo 工作流保留作历史参考；2026-09-10 已按审批移除 Stack 主题，旧工作流不能直接使用。恢复旧版本须恢复原提交及匹配的主题子模块；现行发布使用本文件说明的 Astro 工作流。旧工作流本身尚未删除或禁用。
...
`migration/content-baseline.json` 是旧站历史快照，其中仍记录旧 `.gitmodules`。因此 `check:baseline` / `verify:baseline` 会明确报告该文件缺失：这是本次有记录的预期差异，不自动改写旧基线来消除它。常规 Astro 内容/输出校验不以旧主题为前提。
